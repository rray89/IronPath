...
class PlanGeneratorTest {
    ...
    fun `generate returns one workout per selected day`() {
        val result = generator.generate(PlanningGoal.STRENGTH, setOf(1, 3, 5))
        assertEquals(3, result.workouts.size)
    }

    @Test
    fun `generate with single day returns one workout`() {
        val result = generator.generate(PlanningGoal.HYPERTROPHY, setOf(2))
        assertEquals(1, result.workouts.size)
    }

    @Test
    fun `generate with no selected days preserves an empty weekly plan`() {
        val result = generator.generate(PlanningGoal.HYPERTROPHY, emptySet())
    ...
    fun `generate with all seven days returns seven workouts`() {
        val result = generator.generate(PlanningGoal.GENERAL_FITNESS, setOf(1, 2, 3, 4, 5, 6, 7))
    ...
    fun `generate assigns correct dayOfWeek to each workout`() {
        ...
        val result = generator.generate(PlanningGoal.STRENGTH, days)
    ...
    fun `generate sorts workouts by dayOfWeek ascending`() {
        val result = generator.generate(PlanningGoal.RETURN_TO_ROUTINE, setOf(5, 2, 7))
    ...
    fun `generate plan startDate is next Monday`() {
        val result = generator.generate(PlanningGoal.STRENGTH, setOf(1))
    ...
    fun `generate cycles Strength templates when days exceed pool size of 5`() {
        val result = generator.generate(PlanningGoal.STRENGTH, setOf(1, 2, 3, 4, 5, 6))
    ...
    @Test
    fun `generate produces distinct workout titles for each PlanningGoal`() {
        val goals = PlanningGoal.entries
        val firstTitles =
    ...
    fun `generate exercises belong to their correct workout`() {
        val result = generator.generate(PlanningGoal.HYPERTROPHY, setOf(1, 3))
    ...
    @Test
    fun `every generated exercise round trips through history catalog lookup`() {
        val workouts =
            PlanningGoal.entries.flatMap { goal ->
                val result = generator.generate(goal, (1..7).toSet())
                result.workouts.map { workout ->
                    RecentWorkoutSnapshot(
                        title = workout.title,
                        completedOn = nextMonday,
                        exercises =
                            result.exercises
                                ...
                                .map { exercise ->
                                    RecentLoggedExerciseSnapshot(
                                        name = exercise.name,
                                        completedSetWeightsKg = listOf(1.0),
                                    )
                                },
                    )
                }
        ...
        val summary =
            TrainingHistorySummarizer(exerciseCatalog)
                .summarize(
                    today = nextMonday,
                    workouts = workouts,
                    records = emptyList(),
        ...
        assertEquals(
            workouts.flatMap(RecentWorkoutSnapshot::exercises).map { it.name }.toSet().size,
            summary.exerciseLoads.size,
        )
    }
}
