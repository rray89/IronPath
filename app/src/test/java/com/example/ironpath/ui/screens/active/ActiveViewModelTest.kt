...
class ActiveViewModelTest {
  ...
  /**
   * Runs the test block and then cancels the ViewModel's scope before runTest drains virtual time.
   * Required because ActiveViewModel has an infinite timer loop (delay + while(isActive)); without
   * cancellation, runTest's post-block advanceUntilIdle() would spin it forever and OOM.
   */
    ...
    runTestCancelling {
      ...
      coVerify {
        sessionRepository.startSession(
          match {
            it.sourcePlannedWorkoutId == "workout1" && it.workoutTitle == "Workout workout1"
          },
          match { exercises ->
            exercises.size == 1 &&
  ...
  fun `finishWorkout marks planned workout Completed when completedSets greater than 0`() =
    runTestCancelling {
      ...
      coVerify { planRepository.updateWorkout(match { it.status == WorkoutStatus.Completed }) }
    }
