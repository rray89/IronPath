...
interface PlanDao {
    ...
    // WeeklyPlan
    @Insert suspend fun insertPlan(plan: WeeklyPlan)

    @Update suspend fun updatePlan(plan: WeeklyPlan)
    ...
    // PlannedWorkout
    @Insert suspend fun insertWorkouts(workouts: List<PlannedWorkout>)

    @Update suspend fun updateWorkout(workout: PlannedWorkout)
    ...
    // PlannedExercise
    @Insert suspend fun insertExercises(exercises: List<PlannedExercise>)

    @Query(
        "SELECT * FROM planned_exercises WHERE plannedWorkoutId = :workoutId ORDER BY orderIndex"
    )
